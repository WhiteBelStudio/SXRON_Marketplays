...
type ListingInput = { title?: unknown; description?: unknown; price?: unknown; currency?: unknown; quantity?: unknown; city?: unknown; condition?: unknown; category_id?: unknown; type?: unknown; status?: unknown; images?: unknown };
...
async function uniqueSlug(title:string) {
  ...
  for(let i=0;i<20;i++){const candidate=i===0?base:base+"-"+Math.random().toString(36).slice(2,8);const found=await db.query("SELECT 1 FROM listings WHERE slug=$1 LIMIT 1",[candidate]);if(!found.rowCount)return candidate;}
...
function parseInput(body:ListingInput) {
  const title=String(body.title??"").trim(),description=String(body.description??"").trim(),categoryId=String(body.category_id??"").trim(),currency=String(body.currency??"RUB").trim().toUpperCase(),city=String(body.city??"").trim(),condition=String(body.condition??"").trim(),type=String(body.type??"product").trim(),status=String(body.status??"draft").trim(),price=Number(body.price),quantity=Number(body.quantity??1);
  ...
  const rawImages=Array.isArray(body.images)?body.images:[],images=rawImages.map(item=>String(item??"").trim()).filter(item=>/^https?:\/\//i.test(item)).slice(0,10);
...
export async function GET(request:Request) {
  const url=new URL(request.url),mine=url.searchParams.get("mine")==="true",user=mine?await getCurrentUser():null;
  ...
  try {
    ...
    if(mine){values.push(user!.id);conditions.push("l.owner_id=$"+values.length);}
    else conditions.push("l.status='published'");
    ...
    const minPrice=url.searchParams.get("minPrice"); if(minPrice&&Number.isFinite(Number(minPrice))){values.push(Number(minPrice));conditions.push("l.price >= $"+values.length);}
  ...
  } catch { return NextResponse.json({error:"Не удалось загрузить объявления."},{status:500}); }
}
...
export async function POST(request:Request) {
  ...
  } catch { return NextResponse.json({error:"Не удалось создать объявление."},{status:500}); }
}
