...
type ListingInput = {
  title?: unknown; description?: unknown; price?: unknown; currency?: unknown; quantity?: unknown;
  city?: unknown; condition?: unknown; category_id?: unknown; type?: unknown; status?: unknown; images?: unknown;
};
...
function parseImages(value: unknown): { ok: true; images: string[] } | { ok: false; error: string } {
  ...
  if (images.some((item) => !/^https?:\/\//i.test(item))) {
    return { ok: false, error: "Каждая фотография должна быть URL с http:// или https://." };
  }
...
function parseInput(body: ListingInput) {
  ...
  return { title, description, categoryId, currency, city: city || null, condition: condition || null, type, status, price, quantity, images: parsedImages.images };
}

async function insertImages(client: { query: (text: string, values?: unknown[]) => Promise<unknown> }, listingId: string, images: string[]) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      await insertImages(client, result.rows[0].id, parsed.images);
      await client.query("COMMIT");
      return NextResponse.json({ listing: { ...result.rows[0], images: parsed.images } }, { status: 201 });
    } catch (error) {
