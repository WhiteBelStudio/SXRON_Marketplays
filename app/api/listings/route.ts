...
export async function GET(request: Request) {
  ...
  try {
    ...
    const where = mine
      ? (() => {
          values.push(user!.id);
          return [`l.owner_id=$${values.length}`];
        })()
      : ["l.status='published'"];
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO listings
       (seller_id,category_id,owner_id,type,title,slug,description,price,currency,quantity,city,condition,status)
